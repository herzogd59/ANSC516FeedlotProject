...
metadata$Treatment.ord = factor(metadata$Treatment, c("Nasal+Control", "Nasal+Oxytetracycline", "Nasal+Tulathromycin", "Fecal+Control", "Fecal+Oxytetracycline", "Fecal+Tulathromycin"))
...
#for(ml in my_level){
  ...
  physeq_meta_filtered$Treatment.ord = factor(physeq_meta_filtered$Treatment, c("Control", "Oxytetracycline", "tulathromycin"))
  ...
  ggsave(paste0("~/FeedlotProject/output", ml, "BarPlot_", my_column, ".png"), height = 5, width = 4)
}
...
physeq_deseq_filtered <- tax_filter(
  physeq_deseq,
  min_prevalence = .2,
  prev_detection_threshold = 10,
...
OTU.physeq_Control <- subset_samples(physeq_deseq_filtered, Antibiotic == "Control")
OTU.physeq_Oxytetracycline <- subset_samples(physeq_deseq_filtered, Antibiotic == "Oxytetracycline")
OTU.physeq_Tulathromycin <- subset_samples(physeq_deseq_filtered, Antibiotic == "Tulathromycin")
...
diagdds = phyloseq_to_deseq2(OTU.physeq_Tulathromycin, ~ Tissue)
...
my_colors <- c('#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c','#fdbf6f','#ff7f00','#cab2d6','#6a3d9a','#ffff99','#b15928', "#CBD588", "#5F7FC7", "orange","#DA5724", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14285", "#652926", "#C84248", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "gray", "black")
...
run_deseq2 <- function(my_factor, x, y){
  ...
  DESeq_fig = ggplot(sigtab, aes(x=Genus, y = log2FoldChange, color=Phylum)) + 
    ...
    scale_color_manual(values = my_colors) +  # use all 14 colors
  ...
  ggsave(paste0("~/FeedlotProject/output", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 5, width = 20)
}


run_deseq2("Tissue", "Fecal", "Nasal")
run_deseq2("Treatment", "Control", "Tulathromycin")
run_deseq2("Treatment", "Oxytetracycline ", "Tulathromycin") 
