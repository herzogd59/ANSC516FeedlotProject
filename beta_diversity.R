...
metadata<-read_q2metadata("FeedlotMetadata.txt")
str(metadata)
...
Treatment_colors <- c("Black", "Blue", "Green", "Red", "Purple", "Orange")
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=Treatment)) +
  ...
  xlab("PC1 (32.27%)") +
  ylab("PC2 (22.28%)") +
  scale_color_manual(values=c("Blue", "Black", "Green", "Gray", "Purple", "Orange"), name = "Treatment")
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  geom_point(aes(shape= Tissue)) + #alpha controls transparency and helps when points are overlapping
  geom_point(data=centroids, size = 3) +
  theme_q2r() +
  stat_ellipse(level = 0.95, type = "t") +
  xlab(paste0("PC1 (", round(100*bc_PCoA$data$ProportionExplained[1], digits = 2), "%)")) +
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) 
  scale_color_manual(values=Treatment_colors, name = Antibiotic)
...
Treatment_Pair <- pairwise.adonis2(bc_dm ~ body.site, data = metadata_sub)
