...
meta<-read_q2metadata("ProjectMetadata.txt")
str(meta)
...
ggqqplot(meta$V2, title = "Faith PD")
...
aov.faith.Treatment = aov(V2 ~ Treatment, data=meta)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.faith.Treatment)
...
meta$Treatment.ord = factor(meta$Treatment, c("Nasal+Oxytetracycline", "Nasal+Tulathromycin", "Nasal+Control", "Fecal+Oxytetracycline", "Fecal+Tulathromycin", "Fecal+Control"))
levels(meta$Treatment.ord)

#Plot
boxplot(shannon_entropy ~ Treatment.ord, data=meta, ylab="Shannon")

Shannon_boxplot <- ggplot(meta, aes(Treatment.ord, shannon_entropy)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("output/shannon_entropy_boxplot.png", evenness, height = 3, width = 3)
...
Shannon_se <- ggplot(shannon_summary, aes(Treatment.ord, mean_shannon, fill = Treatment.ord)) + 
  geom_col() + 
...
ggsave("output/ShannonEntropyFinal_se.png", Shannon_se, height = 2.5, width = 3)
...
kruskal.test(shannon_entropy ~ Treatment.ord, data=meta)
...
pairwise.wilcox.test(meta$shannon_entropy, meta$Treatment.ord, p.adjust.method="BH")


#Plot
boxplot(V2 ~ Treatment.ord, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

faith_pd_boxplot <- ggplot(meta, aes(Treatment.ord, V2)) + 
...
ggsave("output/FaithPD_Seperated.png", faith_pd_boxplot, height = 3, width = 3)
...
glm.evenness.time = glm(pielou_evenness ~ Tissue, data=meta)
...
plot(pielou_evenness ~ Tissue, data=meta)
#Add the glm best fit line
plot(pielou_evenness ~ Tissue, data=meta) + abline(glm.evenness.time)
